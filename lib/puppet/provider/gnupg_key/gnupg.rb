...
Puppet::Type.type(:gnupg_key).provide(:gnupg) do
  ...
  def add_key_from_key_server
    if resource[:proxy].nil? or resource[:proxy].empty?
      command = "gpg --no-tty --homedir #{resource[:gnupg_home]} --keyserver #{resource[:key_server]} --recv-keys #{resource[:key_id]}"
    else
      command = "gpg --no-tty --homedir #{resource[:gnupg_home]} --keyserver #{resource[:key_server]} --keyserver-options http-proxy=#{resource[:proxy]} --recv-keys #{resource[:key_id]}"
    end
    begin
  ...
  def add_key_from_key_content
    ...
    command = "gpg --batch --no-tty --homedir #{resource[:gnupg_home]} --import #{path}"
    begin
      output = Puppet::Util::Execution.execute(command, :uid => user_id, :failonfail => true)
    rescue Puppet::ExecutionFailure => e
  ...
  def add_key_at_path
    if File.file?(resource[:key_source])
      command = "gpg --batch --no-tty --homedir #{resource[:gnupg_home]} --import #{resource[:key_source]}"
      begin
        output = Puppet::Util::Execution.execute(command, :uid => user_id, :failonfail => true)
      rescue Puppet::ExecutionFailure => e
  ...
  def add_key_at_url
    ...
    case uri.scheme
    when /https/
      command = "wget -O- #{resource[:key_source]} | gpg --batch --no-tty --homedir #{resource[:gnupg_home]} --import"
    when /http/
      command = "gpg --batch --no-tty --homedir #{resource[:gnupg_home]} --fetch-keys #{resource[:key_source]}"
    when 'puppet'
